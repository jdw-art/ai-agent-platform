...
class ExampleService:
    ...
    @staticmethod
    async def create_from_feedback(db: AsyncSession, trace_id: str, feedback_tryp: str, user_id: str = None) -> Optional[ChatBIExample]:
        """
        根据反馈，从trace中提取成功的 SQL 并创建经验记录
        """
        try:
            # 1. ORM 方式查找 trace
            import re
            ...
            for t in traces:
                ...
                if "sql" in t_name or (t.event_type == "tool" and "sql" in str(t_input_raw).lower()):
                    trace = t
                    try:
                        # 增加空值检查，防止 json.loafs 抛出Exception value 错误
                        if not t_input_raw:
                            continue
    ...
    async def _enhance_example_with_llm(example_id: int):
        ...
        async with AsyncSessionLocal() as db:
            try:
                ...
                res_his = await db.execute(stmt_hist)
                histories = res_his.scalars().all()
    ...
    @staticmethod
    async def ensure_chatbi_ample_kb_id() -> str:
        """
        确保 chatbi-example-meta 知识库在 RAGFlow 中存在，并将其 ID 写入 system_configs 中。
        如果不存在或失效，则自动创建并写入。
        """
        target_kb_id = await ConfigService.get("chatbi_sample_knowlege_base")
    ...
    async def sync_to_ragflow(example_id: int):
        """
        同步经验到 RAGFlow，使用增强后的 Markdown 模版
        """
        async with AsyncSessionLocal() as db:
            client = RagFlowClient()
            try:
                ...
                target_kb_id = await ExampleService.ensure_chatbi_ample_kb_id()
    ...
    @staticmethod
    async def _cleanup_romote_by_name(client: RagFlowClient, kb_id: str, file_name: str):
        """
