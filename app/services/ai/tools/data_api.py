...
def validate_sql(sql: str, dialect: str = "clickhouse") -> Optional[str]:
    """
    验证输入的 SQL 字符串，确保其安全性和策略合规性。
    使用 sqlglot 进行强大的语法检查。
    支持只读语句：SELECT、WITH...SELECT、EXPLAIN、SHOW、DESCRIBE/DESC。
    """
    sql_clean = sql.strip()
    if not sql_clean:
        return "Empty SQL query"

    # 在关键字检查之前去除前导 SQL 注释（/* ... */ 块和 -- 行）
    ...
    try:
        ...
        # 对抽象语法树 (AST) 中的危险命令进行深度检查
        # 允许安全的只读命令；阻止 OPTIMIZE、KILL 等系统命令。
...
async def call_external_sql_api(sql: str, data_source: Optional[str]) -> str:
    ...
    if execution_mode == "local":
        ...
        if isinstance(adapter, OracleAdapter):
            ...
            # MySQL / ClickHouse 等其他数据库，直接返回原始 SQL
            clean_sql = sql.strip().rstrip(";")
