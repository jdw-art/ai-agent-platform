...
async def memory_search(
    ...
    """检索当前用户跨会话历史：摘要列表或指定会话消息明细。当用户问「今天我们聊了啥」「最近/上次聊了什么」「有没有讨论过」「回顾历史对话」时必须优先调用；
    新会话 messages 为空不代表从未聊过。禁止未调用本工具就声称「第一次对话」或编造历史。scope=summary 查跨会话摘要（有 query 则语义检索，支持解析昨天、今天等时间词）；scope=history 需 conversation_id 拉该会话 Redis 明细；scope=both 先摘要再明细。user_id 由系统自动隔离，勿传入他人 ID。
