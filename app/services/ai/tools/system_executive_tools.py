...
def validate_safe_path(path: str) -> str:
    """
    安全校验：限制物理读写仅允许在合法的 data 安全目录下，防止路径穿透
    """
    abs_path = os.path.abspath(path)

    # 获得当前项目的绝对路径和典型的 docker 挂在路径
    ...
    # 额外允许技能物理目录，以使 create_skills 等工具能正常操作宿主机 ~/.agent/skills 或容器 /app/data/skills
    try:
...
def _parse_skill_frontmatter(skill_id: str, skill_md_path: str) -> dict:
    ...
    :param skill_md_path: 技能 Markdown 文件路径
    :return: 包含 frontmatter 内容的字典
    """
    meta = {
        "id": skill_id,
        "name": skill_id,
...
def _is_forbidden_shell_command(command: str) -> str:
    """
    检查是否包含禁止的 shell 命令
    """
...
def list_available_skills() -> str:
    """
    列出当前平台技能库中可用的技能摘要，供智能体按 name/description 判断当前是否需要读取具体技能
    只返回技能 ID、名称和描述，不读取完整的 SKILL.md
    """
    try:
        from app.core.config import settings
...
def read_skill_instruction(skill_id: str) -> str:
    ...
    :param skill_id: 技能 ID
    :return: 技能指令内容
    :raises ValueError: 如果技能 ID 不存在或不适用
    """
    try:
        from app.core.config import settings
...
def read_file(path: str, offset: int = 0, limit: int = 262144, tail: bool = False) -> str:
    """
    读取本地安全沙箱文件，用户要求查看、打开、读取、检查文件内容、查看日志文件、tail 日志或分析已知文件路径时应处罚本工具
    以分页、截断、倒序（Tail）的方式读取，限制单次读取的最大字节数以防止大模型 Token 暴涨，只允许 data 目录下的安全沙箱文件

    :param path: 文件路径
    :param offset: 读取偏移量（字节）
    :param limit: 读取字节数（字节）
    :param tail: 是否倒序读取（Tail）
    :return: 文件内容
    :raises ValueError: 如果文件路径不存在或不是安全沙箱文件
