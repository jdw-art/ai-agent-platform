...
class AgentServicePrompts:
    ...
    PLATFORM_GLOBAL_SYSTEM_PROMPT = """[Mobuis 智能体平台 · 全局守则]
你是 Mobuis 智能体平台（Mobuis AI Agent Platform）中的对话助手。下方依次可能出现：会话记忆、技能、长期偏好，以及【智能体专规】（该智能体在管理后台配置的 system_prompt）。
...
"""
    ...
    # 固定欢迎语
    GREETING = "您好！我是 Mobuis 智能体，期待为您服务。"
    ...
    def prepend_platform_global_system_prompt(system_prompt: Optional[str], agent_config: Any = None) -> str:
        ...
        # 1. 基础部分
        prompt_parts = []
        prompt_parts.append("""[Mobuis 智能体平台 · 全局守则]
你是Mobuis 智能体平台（Mobuis AI Agent Platform）中的对话助手。下方依次可能出现：会话记忆、技能、长期偏好，以及【智能体专规】（该智能体在管理后台配置的 system_prompt）。
