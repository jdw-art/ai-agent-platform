...
def looks_like_compound_query_with_viz(user_question: str) -> bool:
    """同一句里既要查数，又要可视化分析：新数据查询，不用复用上一轮结果"""
    q = (user_question or "").strip().lower()
    if not q:
        return False
...
def _build_history_context(history: Optional[List[Dict[str, str]]]) -> str:
    ...
    - 按照用户/助手角色分组，每个角色最多保留6轮对话。
    - 每轮对话内容会被压缩，最多保留200个字符。
    """
    if not history:
        return ""
...
class IntentService:
    ...
    async def identify_intent(
        ...
        """
        分析用户输入并返回结构化的 IntentResponse。
        - param user_input: 用户输入的自然语言文本
        - param llm: 自定义 LLM 实例，默认使用配置中的默认模型
        - param history: 上下文对话历史，默认使用空列表
        - return: 意图识别结果
        """
        ...
        # 系统提示词内置在代码中，并用 .replace 注入占位符，避免 few-shot 中的json
        # 花括号被 ChatPromptTemplate 当成模版变量解析
