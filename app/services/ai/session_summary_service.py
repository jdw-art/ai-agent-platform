...
class SessionSummaryService:
    ...
    async def should_run(user_id: str, conversation_id: str, assistant_content: str) -> bool:
        ...
        # 2. 读取配置，至少等待 debounce_seconds 秒或 debounce_turns 轮后运行一次
    ...
    async def merge_session_summary(
        ...
        try:
            ...
            await DailySummaryService.refresh_for_data(uid)

            max_session = await MemoryConfigService.get_int("memory_summary_max_sessions", 50)
            all_items = await MemoryIndexService.list_summaries(uid, limit=max_session + 10)
            if len(all_items) > max_session:
                for old in all_items[max_session:]:
