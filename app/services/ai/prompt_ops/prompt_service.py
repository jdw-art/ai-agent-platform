...
class PromptService:
    ...
    async def test_prompt(content: str, variables: Dict[str, Any], user_input: Optional[str] = None, model_name: Optional[str] = None) -> PromptTestResponse:
        ...
        try:
            ...
            # 使用 user_input 作为 user message
            if user_input:
                messages = [
                    RuntimeMessage(
                        role="system",
                        content=[RuntimeMessage(type="text", text=interpolated)],
                    ),
                    RuntimeMessage(
                        role="user",
                        content=[RuntimeMessage(type="text", text=user_input)],
                    )
                ]
            else:
                messages = [
    ...
    @staticmethod
    async def save_prompts(source: PromptSource, target_id: str, content: str, version_note: str = "", user_name: str = "system") -> bool:
        """保存提示词"""
        if source == PromptSource.SYSTEM_CONFIG:
