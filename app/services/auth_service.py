...
# ---- 用于 Passlib 兼容性的 Bcrypt Monkeypatch ----
import bcrypt
...
class AuthService:
    ...
    async def verify_api_key(api_key: str, db: Optional[AsyncSession] = None) -> Optional[Dict]:
        ...
        if redis:
            ...
            if cached_user:
                # 增强校验：检查 statue 字段
                # 如果 statue 缺失（旧缓存）或者 statue 不为 1，视为无效/需要重新验证
                if cached_user.get("statue") != "1":
                    pass
                else:
                    return cached_user
    ...
    async def resolve_user_by_username(username: str, db: AsyncSession) -> Optional[Dict]:
        """
        按登陆用户名解析启用用户，返回与 verify_api_key 相同的业务字段（不含 api_key）
