...
class LoginRequest(BaseModel):
    """登陆请求入参"""
    api_key: Optional[str] = Field(None, description="API 密钥", examples="S63B...")
...
@router.post("/sso/login", summary="SSO 用户登陆")
async def sso_login(
    ...
    if result["status"] == "success":
        ...
        if not api_key:
            raise HTTPException(500, "用户 API 密钥不存在")
...
@router.post("/logout", summary="用户退出登陆")
async def logout(
    response: Response,
    api_key: Optional[str] = Header(None, alias="X-API-Key")
):
    """
    用户退出登陆接口，并清除 Cookie 和 Redis 缓存
    """
    if api_key:
